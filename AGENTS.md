...
2. **Never hand-edit the `?v=HASH` cache-busting params** on `<script>` tags in `frontend/index.html`, or `APP_BUILD_DATE` in `frontend/modules/config.js`, when editing a `frontend/modules/*.js` file. Both are regenerated by `frontend/release_prep.py`, which the maintainer runs right before a prod release. Only touch them if asked directly.

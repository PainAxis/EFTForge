...
// Shown once, the first time a user flips the toggle on, so they can't miss
// that this is a third-party API we don't control before they start relying on it.
const _BP_DISCLAIMER_ACK_KEY = "eftforge_imggen_disclaimer_ack";

// True when an admin has globally disabled image generation server-side, or
// when the desktop app runs in local mode (previews are generated by the
// EFTForge.com service, so they can never work locally).
...
function toggleImgGen() {
    ...
    // Turning it on for the first time ever - block on the disclaimer before
    // actually flipping anything. _bpApplyToggle(true) only runs if they ack it.
    if (!_bpEnabled && localStorage.getItem(_BP_DISCLAIMER_ACK_KEY) !== "1") {
        _bpShowDisclaimerModal(() => {
            localStorage.setItem(_BP_DISCLAIMER_ACK_KEY, "1");
            _bpApplyToggle(true);
        });
        return;
    }
...
// Builds the "third-party service, don't yell at us" disclaimer modal and
// only calls onAck() if the user actually clicks through it.
function _bpShowDisclaimerModal(onAck) {
    const { t, tFmt } = EFTForge.lang;
    const overlay = _createModalOverlay("imggen-disclaimer-modal", t("bp.disclaimerTitle"), {
        closeId: "imggen-disclaimer-close",
        bodyId:  "imggen-disclaimer-body",
        maxWidth: "460px",
    });
    if (!overlay) return;

    const imageGenLink = `<a href="https://image-gen.tarkov-changes.com/build" target="_blank" rel="noopener noreferrer">${t("bp.imageGenLinkText")}</a>`;
    const bilibiliLink = `<a href="https://space.bilibili.com/650421245" target="_blank" rel="noopener noreferrer">${t("bp.bilibiliLinkText")}</a>`;

    // Same warning-type triangle as the first-visit migration notice modal, enlarged.
    const warnTriangle =
        '<svg width="320" height="320" viewBox="0 0 20 20" fill="none" style="color:#e74c3c;">' +
    ...
    document.getElementById("imggen-disclaimer-body").innerHTML = `
        <div class="modal-section">
            <div style="display:flex;justify-content:center;padding:4px 0 8px;">${warnTriangle}</div>
            <p style="color:#ccc; font-size:14px; margin:0 0 12px 0; line-height:1.5;">${tFmt("bp.disclaimerBody1", { link: imageGenLink })}</p>
            <p style="color:#ccc; font-size:14px; margin:0 0 12px 0; line-height:1.5;">${t("bp.disclaimerBody2")}</p>
            <p style="color:#ccc; font-size:14px; margin:0 0 12px 0; line-height:1.5;">${t("bp.disclaimerBody3")}</p>
            <p style="color:#ccc; font-size:14px; margin:0 0 16px 0; line-height:1.5;">${tFmt("bp.disclaimerBody4", { link: bilibiliLink })}</p>
            <div class="modal-row">
                <button class="modal-btn full-width" id="imggen-disclaimer-cancel">${t("modal.cancel")}</button>
                <button class="modal-btn primary full-width" id="imggen-disclaimer-ack">${t("bp.disclaimerAck")}</button>
            </div>
        </div>
    `;

    document.getElementById("imggen-disclaimer-cancel").addEventListener("click", () => overlay.remove());
    document.getElementById("imggen-disclaimer-ack").addEventListener("click", () => {
        overlay.remove();
        _bpShowChecklistModal(onAck);
    });
}

// Second gate: forces an explicit tick on every point from the disclaimer
// before the enable button will even respond. onConfirm only fires once all
// boxes are checked and the user clicks through.
function _bpShowChecklistModal(onConfirm) {
    const { t } = EFTForge.lang;
    const overlay = _createModalOverlay("imggen-checklist-modal", t("bp.checklistTitle"), {
        closeId: "imggen-checklist-close",
        bodyId:  "imggen-checklist-body",
        maxWidth: "460px",
    });
    if (!overlay) return;

    const itemsHtml = [1, 2, 3, 4].map(n => `
        <label class="imggen-check">
            <span class="imggen-check-box-wrap">
                <input type="checkbox" class="imggen-checklist-box imggen-check-input" />
                <span class="imggen-check-box"><img src="./assets/images/icon-fir.png" class="imggen-check-tick" alt="" /></span>
            </span>
            <span class="imggen-check-label">${t(`bp.checklistItem${n}`)}</span>
        </label>
    `).join("");

    document.getElementById("imggen-checklist-body").innerHTML = `
        <div class="modal-section">
            ${itemsHtml}
            <div class="modal-row" style="margin-top:4px;">
                <button class="modal-btn full-width" id="imggen-checklist-cancel">${t("modal.cancel")}</button>
                <button class="modal-btn primary full-width" id="imggen-checklist-confirm" disabled style="opacity:0.5; cursor:not-allowed;">${t("bp.checklistEnable")}</button>
            </div>
        </div>
    `;

    const boxes      = [...overlay.querySelectorAll(".imggen-checklist-box")];
    const confirmBtn = document.getElementById("imggen-checklist-confirm");
    ...
    boxes.forEach(box => box.addEventListener("change", refreshConfirmState));

    document.getElementById("imggen-checklist-cancel").addEventListener("click", () => overlay.remove());
